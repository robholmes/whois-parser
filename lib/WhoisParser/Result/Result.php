            ...
            foreach ($targetArray as $key => $type) {
                if ($targetArray[0] == 'contacts' && $key == 1 && sizeof($targetArray) == 2) {
                    ...
                    if (! isset($this->contacts->{$this->lastHandle}[$this->lastId])) {
                        $this->contacts->{$this->lastHandle}[$this->lastId] = new \WhoisParser\Contact();
                    }
                    ...
                    if (! isset($element->$type)) {
                        switch ($targetArray[0]) {
                            case 'contacts':
                                ...
                                array_push($element->$type, new \WhoisParser\Contact());
                                break;
                            case 'registrar':
                                $element->$type = new \WhoisParser\Registrar();
                                break;
                            default:
